...
def _sync_fund_impl(
    ...
    #
    # REGLA PERMANENTE (contrato v2): un fondo CERRADO no se recategoriza al reanalizar.
    ...
    try:
        ...
        _cerrado = False
    if _cerrado:
        log(f"[SYNC] {isin} CERRADO → no se recategoriza (contrato v2). "
            f"Reabrir: python -m tools.fund_estado --set {isin} pendiente")
