...
| **W** | **2026-06-17** | **Taxonomía catálogo + quant diario Morningstar + MyInvestor vía cowork + Excel espejo**: (1) **Taxonomía** (`tools/sync_taxonomy.py`) desde el Excel maestro BDD + Morningstar: `tipo_activo` (con **Monetario**), `geografia` (regla **Developed/ACWI** en RV: Developed solo si se indica/MSCI World, resto RV global→ACWI; fallback a valores existentes si el fondo no está en el Excel), `categoria_rf` (Gubernamental/Corporativa/Flexible-Diversificada de Morningstar), `plazo` (CP/MP/LP, campo aparte por ser subjetivo), `srri`, `caracteristicas_especiales` (multi-tag: HY/ILS/Indexado/Sectorial/SmallCaps/CubreDivisa/REITs/Retorno absoluto), `distribucion` por clase (nombre+folleto, luego MyInvestor robusto). Columnas nuevas Supabase (DDL manual). Constraint `chk_tipo_activo` exige plural (Mixtos/Alternativos/Materias_primas). Clasificaciones del Excel: 10/11 reclasificaciones validadas, Dunas Flexible→Top, Schroder→Bueno. (2) **Quant unificado** (`tools/morningstar_daily.py`): serie diaria de NAV por ISIN (misma key `2nhcdckzon`/`idtype=Isin` que el fund-dashboard) → rentab. por año natural, CAGR, volatilidad (mensual anualiz.), max DD, peor/mejor año, rentab. por plazo → `fund_groups.rendimiento_jsonb` (146/173). CAGR Magallanes 9.76%=MyInvestor. (3) **MyInvestor vía cowork**: VERIFICADO que `claude -p` (Claude Max) SÍ accede al conector claude.ai MyInvestor (el pipeline Python NO). Skill `.claude/skills/myinvestor-enrich` (busca por nombre→match ISIN→distribución robusta Acc/Reparto + disponibilidad + allocation + sectores + rentab año natural + SRRI + rating + mínimo + URLs docs → `myinvestor_data.json`) + `tools/myinvestor_consume.py` (vuelca a Supabase: funds.distribucion/broker/mínimo, fund_groups.portfolio_metrics_jsonb.myinvestor). Cableada en `analizar_fondo.bat` paso 5.5 (`--allowedTools` con el MCP). (4) **`tools/derive_taxonomy.py`**: deriva tipo/geo/categoria_rf/srri/tags de MyInvestor+Morningstar (sin Excel) para fondos NUEVOS, rellena solo vacíos. (5) **`tools/export_bdd_excel.py`**: Excel maestro espejo 100% de Supabase (296 filas, una por clase) con toda la taxonomía+quant+MyInvestor. **Modelo de sync**: Supabase = fuente de verdad (editas ahí), Excel = espejo de solo-lectura regenerado ("ver qué recomendar"). Sin fondos "colgados": Morningstar (serie diaria+screener por ISIN) = red universal; MyInvestor = capa extra donde está. Tests `tests/test_reconcile_fund_groups.py` etc. | No (aditivo) |
| **S** | **2026-06-16** | **Vínculo manual de clases (mismo fondo, ISINs distintos)**: el agrupado automático por nombre+gestora (`fund_groups`) falla cuando los nombres no coinciden exactamente. Nuevo `tools/class_links.py` (registro `data/fund_class_links.json`: `alias_isin → primary_isin`) para vincular A MANO una clase a un fondo ya analizado: la clase NO se re-analiza, comparte el análisis del primario y se ABSORBE como sub-clase (deja de ser fila propia en el catálogo). Endpoints `web_server`: `GET/POST/DELETE /api/class-links`. **Catálogo colapsa por `fund_group_id`** (`collapseByFundGroup`): las clases del mismo fondo (mismo grupo, ya en Supabase) se agrupan SOLAS en una fila — la del primario (más track-record: `has_qualitative_analysis` + `años_antiguedad`) — y las demás se cuelgan en `_classes` (chip "+N clases"). Funciona en local Y en **público** (el dato viene de Supabase). Esto cubre el agrupado automático (clases con mismo nombre+gestora). Para clases con nombres que NO coinciden: vínculo manual "🔗" → `applyManualLinks` (override `fund_group_id` en cliente) + **escritura a Supabase** (`sync_link_to_supabase`: la fila `funds` del alias hereda el `fund_group_id` del primario) → el catálogo público también agrupa. Anti-ciclo + cadenas a la raíz. Pendiente: la pestaña Evolución/cuantitativa sigue siendo la del primario (no per-clase aún); la URL directa de un alias no redirige. Tests `tests/test_class_links.py` (3). | No (aditivo) |
| **T** | **2026-06-16** | **Reconciliación de clases por ISIN (señal autoritativa = folleto, no nombre; online)**: el agrupado determinista (`uuid5(gestora::nombre_base)`) partía clases del MISMO fondo cuando el `nombre_base` no coincidía exacto → en el catálogo una clase clasificada y otra no (caso Trojan IE00BYV18N80 vs IE00B6T42S66). **Corrección de método (feedback Rafa)**: lo que marca "mismo fondo, otra clase" NO es el nombre sino el **ISIN**. La señal autoritativa es la lista `clases[].isin` que ya extraemos del **folleto/AR** al analizar (con divisa, comisión, año — como el listado de clases de Morningstar, pero de la fuente primaria). `tools/reconcile_fund_groups.py` reescrito: modo por defecto = **`reconcile_by_isin`** (agrupa los ISINs que el folleto lista juntos) + **`push_class_isins_to_supabase`** que vuelca esos ISINs a `fund_groups.class_isins_known` (ground-truth ONLINE). El método de nombre queda como `--name-heuristic` opt-in (frágil). Intento previo por nombre (prefijo de tokens) fusionó 2 **paraguas** por error (Dodge & Cox Worldwide *EUR* truncado, Aegon Absolute Return *H* truncado — el sub-fondo no aparece en el nombre) → **revertidos**. Se mantienen 12 merges seguros (mismo nombre completo + sufijo de clase: Groupama, BlueBay, SIH, Candriam, Dunas Flexible, Dunas Audaz, R-Co, Sextant, Sigma, Vanguard Small, iShares Developed/Emerging) + 2 vínculos manuales (Trojan, Avantage). Forward (punto 2): `resolve_group_for_new_fund(..., new_isin)` consulta `class_isins_known` ONLINE → una clase NUEVA de un fondo ya analizado se agrupa sola POR ISIN (validado: FR001400CEH2 con nombre distinto → grupo Sextant). Cableado en `sync_to_supabase` (insert usa el lookup; cada análisis vuelca su `clases[].isin` a `class_isins_known`). 100% online. **Clases como filas + desplegable**: `populate_fund_classes` inserta cada clase del folleto como fila `funds` compartiendo `fund_group_id`+cualitativo del primario, con SUS cuantitativos propios (`divisa`, `comision_gestion_pct`, `ter_pct`, `fecha_creacion_clase`, `importe_minimo_eur`); NO pisa clases que ya sean fondos analizados (solo asegura grupo). CLI `--insert-classes`; cableado en `sync_to_supabase` (nuevos análisis lo hacen solos). Backfill: 5 clases (Sextant CEH2/CEI0/CEJ8/CEL4 + DNCA LU1791427914). Catálogo (`collapseByFundGroup`): `_all_classes` = primario + clases; chip "+N clases ▾" clicable (`toggleClasses`, `stopPropagation`) abre fila desplegable con ISIN/clase/divisa/año inicio/comisión gestión/TER por clase. Tests `tests/test_reconcile_fund_groups.py` (7). **OJO**: nombres de paraguas en catálogo vienen truncados → nunca agrupar paraguas por nombre, solo por ISIN. Pendiente: cuantitativos/NAV per-clase reales (ahora la clase hereda métricas del grupo; solo difieren divisa/comisión/año del folleto). | No (aditivo) |
| **V** | **2026-06-16** | **Datos cuantitativos Morningstar en los análisis (screener público, sin login)**: misma vía que las clases (`lt.morningstar.com` key `klr5zyak8x`). `tools/morningstar_quant.py:fetch_quant(isin)` → **Medalist rating** (Gold/Silver/Bronze), estrellas, sustainability, **riesgo a 3/5/10A** (alpha, beta, sharpe, **sortino**, **max drawdown**, volatilidad, IR, R²), **valoración** (P/E, P/B, **cap. media**), **renta fija** (calidad crediticia, maturity, duración), **comisiones** (TER, gestión, entrada, mínimo), patrimonio, y **serie NAV** (`fetch_nav`). Escrito en `analisis_cuantitativo.morningstar` por `quant_enrichment.enrich_fund` (junto a Yahoo; Morningstar autoritativo donde es mejor). Render dashboard: CARTERA → card Rating Morningstar (medalist+estrellas), riesgo por plazo con sortino+maxdd, cap. media, card Renta fija, **card Captura (5A)** alcista/bajista vs línea 100%, **Exposición sectorial vs índice** (fondo vs ETF benchmark, dif. pp); RESUMEN → tabla de comisiones. **NO accesible** (API SAL, 401 con cualquier header): holdings individuales, % por rango de capitalización, pesos sector/país detallados, **fecha de compra por posición** (premium), documentos PDF. Geografía vs benchmark NO hecha (Yahoo no da región de fondo). Backfill+regen de los ~41 fondos. | No (aditivo) |
| **U** | **2026-06-16** | **Listado completo de clases por fondo — fuente híbrida (decisión Rafa: CNMV para ES + Morningstar para resto)**: cobertura previa pésima (2/41 fondos con lista buena, 5 rota, 34 sin nada). **ES → CNMV** (`tools/cnmv_classes.py`, autoritativo y gratis sin login): cruza `vista=3` ("Clases de participaciones": Nº/Denominación/Fecha alta/ISIN) con `vista=34` ("Comparativa comisiones": comisión gestión/depósito/mínimo por clase) del portal CNMV por NIF. Devuelve `[{isin, nombre_clase, divisa:EUR, comision_gestion_pct, fecha_inicio, min_suscripcion}]` → `populate_fund_classes`. **25 clases ES insertadas** (Magallanes European/Iberian/Microcaps 4/4/2, Cobas Internacional 4, Dunas Flexible/Equilibrado/Prudente 5/5/4, Avantage 2, Bankinter Japón 2, Gamma 2, Sigma 2). HTTP 400 en `vista=3` = fondo monoclase (no inventar). CLI `python -m tools.cnmv_classes <NIF>` o `--all [--apply]`. `populate_fund_classes` ahora también mantiene `fund_groups.class_isins_known` (ground-truth ONLINE por ISIN, sea cual sea la fuente). Cableado en `sync_to_supabase`: fondos ES leen NIF de `cnmv_data.json` y vuelcan clases CNMV al sincronizar. **Resto (no-ES) → Morningstar API pública** (`tools/morningstar_classes.py`): NO el conector MCP (no existe) ni scraping HTML (snapshot bloqueado), sino el **screener** `lt.morningstar.com/api/rest.svc/klr5zyak8x/security/screener` (JSON, no bloqueado): busca por ISIN → `FundId`, filtra `FundId:IN:{FundId}` → todas las clases con ISIN, divisa, OngoingCharge (TER), ManagementFee (comisión), InceptionDate. **Agrupa por FundId (nivel sub-fondo)** → resuelve el problema de paraguas (Dodge & Cox, Aegon, Trojan) que el nombre no podía. Regla Rafa: **>5 clases → solo EUR** (garantizando el ISIN buscado). Validado: Trojan IE00B6T42S66 → 6 clases I/O/X Acc/Inc; Sextant → 6; Wellington → 6; ML Alpha Fixed Income → 5. Cableado en `sync_to_supabase` (rama no-ES). Tests `tests/test_cnmv_classes.py` (2) + `tests/test_morningstar_classes.py` (2). | No (aditivo) |
| **R** | **2026-06-16** | **Fix KPI "años desde inicio" + chat por fondo user-friendly**: (R1) el KPI "Años desde inicio" salía "—" en ~la mitad de los fondos porque `kpis.anio_creacion` viene null (23/43); ahora si falta se extrae el año de la **fecha de inicio ya resuelta** por el header (`_hdr_resolve_fecha_inicio`) → `año_actual − año_inicio` (floor). Magallanes: 11. (R2) el chat por fondo usaba un **servidor aparte** `chat_server.py` (FastAPI, puerto 8899, manual, Gemini) → mostraba "Ejecutar: python chat_server.py". Unificado en `tools/web_server.py` (mismo backend + `API_BASE` que el feedback): endpoints `/api/chat/<isin>/info` y `/api/chat/<isin>` con **Anthropic Haiku + prompt caching** (sin dependencia de Gemini). Lógica de carga de docs + system prompt extraída a `tools/chat_context.py` (sin FastAPI). JS del chat: `API_BASE` de localStorage, historia en cliente, respuesta JSON, mensaje amigable cuando no hay servidor. Validado en vivo (respuesta Haiku correcta desde los documentos). `chat_server.py` se mantiene como legacy standalone. | No (aditivo) |
| **Q** | **2026-06-08** | **Robustez del nombre de fondo (gate anti-basura)**: nuevo `tools/fund_name_utils.is_valid_fund_name(name, isin) → (ok, motivo)` que ACEPTA nombres legales (incl. paraguas largos tipo "ROBECO CAPITAL GROWTH FUNDS - …") y RECHAZA ISIN, ISIN embebido (token 12-char con dígito + `\b`, evita falsos positivos con nombres largos), prosa (>16 palabras, varias frases, empieza minúscula/conector), etiquetas/stubs ("Información del fondo", "Nombre del fondo"…), fragmentos. `name_recovery.needs_recovery` ahora usa el validador → la recuperación del pipeline (de `analyst_synthesis` via regex+Haiku) se dispara también con nombres-prosa/etiqueta/erróneos, no solo vacío/==ISIN/<5 (era el hueco por el que se colaban). Gate en escritura: `intl_extractor_v2._merge_target_fund_identity` solo guarda el nombre del identity card del regulador si es válido. Auditoría: estado almacenado casi limpio (4/43, mayoría nombres legales largos legítimos + 1 vacío) → el fallo era en la resolución durante el análisis. Tests `tests/test_fund_name_validation.py` (3). | No (aditivo) |
| **P** | **2026-06-08** | **Perfil cuantitativo estilo-Morningstar (Yahoo + cálculo propio)**: Morningstar bloquea el acceso automatizado (CloudFront 403 a httpx Y a Chrome headless/headed — verificado). Fuente alternativa: `tools/quant_enrichment.py` con **Yahoo Finance** (ISIN→símbolo→crumb→quoteSummary) para **style box** (decode `styleBoxUrl`), **sectores**, **valoración Fondo vs Índice** (P/E, P/B, P/S, P/CF de `equityHoldings`, recíproco→ratio, índice vía ETF benchmark) y **riesgo/rentabilidad por holding-period** (3/5/10A: volatilidad, Sharpe, beta, alpha, R², Treynor + retornos anualizados, de `fundPerformance`); más **capture ratios calculados** (serie NAV vs benchmark, def. Morningstar) y **rotación** (de `cuantitativo.serie_rotacion` CNMV, filtrando huecos 0.0). Escrito en `output.json["analisis_cuantitativo"]` por `enrich_and_save` (Python puro, best-effort, cableado en `consume_all_cowork_pipeline`). Render: panel "Perfil cuantitativo" en rejilla 2×3 de tarjetas en la pestaña **Cartera** (antes de la tabla de posiciones). NO obtenible (propietario/bloqueado): Perfil Factor (Momentum/Calidad/Liquidez), Active Share. Validado en vivo (Magallanes): style box Mid·Value, P/B 1.16 vs 2.28, Sharpe 3A 0.95, capture 95/87, rotación media 19%. Tests `tests/test_quant_enrichment.py` (5). | No (aditivo) |
| **O** | **2026-06-08** | **Info de vídeos/entrevistas (transcripts YouTube)**: `tools/youtube_transcript.py` convierte una URL de YouTube en el TEXTO HABLADO del vídeo (subtítulos auto/manuales) vía `youtube-transcript-api` (gratis, sin key, multi-idioma con fallback). Enganchado en `readings_collector._fetch_and_extract`: si la URL es de YouTube intenta el transcript (prefijo `[TRANSCRIPCIÓN DE VÍDEO/ENTREVISTA]`) antes del fetch HTML; si no hay subtítulos/lib/red → cae al HTML como antes (sin regresión). Así el análisis ingiere lo que el gestor DICE en entrevistas/podcasts, no solo que existen. Anti-contaminación: `_validate_relevance` exige que el transcript mencione el fondo/gestora (filtra entrevistas genéricas). Módulo defensivo (APIs 0.6.x estática y 1.x instancia). Tests `tests/test_youtube_transcript.py` (3, parsing offline). Dep en requirements. **Discovery de vídeo mejorado** en `readings_collector`: queries DEDICADAS `site:youtube.com` con keywords de vídeo en el idioma correcto (ES: entrevista/podcast/webinar/conferencia/charla; INT: idioma local vía `LANG_KEYWORDS` + interview EN) a nivel fondo + **nombre COMPLETO de gestora** (no token suelto: "Magallanes" traía al explorador/homónimos; "Magallanes Value Investors" no) + por-gestor (×3). Validado en vivo con Serper sobre Magallanes: 12/12 resultados YouTube, entrevistas reales de Iván Martín, contaminación del explorador eliminada al usar nombre completo. | No (aditivo) |
