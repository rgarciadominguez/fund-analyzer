...
| **T** | **2026-06-16** | **Reconciliación de clases partidas en distintos `fund_group_id` (online, todos los fondos)**: el agrupado determinista (`uuid5(gestora::nombre_base)`) partía clases del MISMO fondo cuando el `nombre_base` no coincidía exacto (nombres basura del bug Q, o sufijos de clase distintos) → en el catálogo una clase clasificada y otra no (caso Trojan IE00BYV18N80 vs IE00B6T42S66). Nuevo `tools/reconcile_fund_groups.py`: detecta splits por **prefijo de tokens dentro de la misma gestora** (NO por "primeros 4 tokens iguales" — eso fusionaba fondos DISTINTOS: Amundi IS Core MSCI *Europe* vs *Emerging Markets*, Vanguard Short-Term *Corporate* vs agregado, Amundi Index Solutions *Government Bond* vs *REIT*; 3 falsos positivos cazados). Sobre los 178 fondos de Supabase: **14 merges verificados aplicados** (Groupama, Dunas Flexible/Audaz, BlueBay, Aegon, SIH, Dodge&Cox, Candriam, Vanguard Small, R-Co, Sextant CEG4/CEK6, Sigma, iShares Developed/Emerging) reasignando la clase alias al `fund_group_id` del primario (con `has_qualitative_analysis`). Las clases con base no agrupable (1-2 tokens: Trojan 'O', Avantage A/B) → **vínculo manual** (`class_links` + `sync_link_to_supabase`). Forward: `resolve_group_for_new_fund(client, nombre, gestora, fallback_gid)` cableado en `sync_to_supabase` insert → una clase NUEVA de un fondo ya en Supabase reutiliza el grupo de su hermana (prefijo-compatible) y se agrupa sola sin depender del nombre_base exacto. 100% online (escribe en Supabase → catálogo público colapsa). DRY-RUN final: 0 splits recuperables. Tests `tests/test_reconcile_fund_groups.py` (4). | No (aditivo) |
